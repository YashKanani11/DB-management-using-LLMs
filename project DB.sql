...
as total_revenu from (select sum(price*stock_quantity) as total_amount, t_shirt_id 
...
SELECT SUM(price * stock_quantity * ((100 - COALESCE(discounts.pct_discount, 0)) / 100)) AS total_revenue
FROM t_shirts
